...
JNIEXPORT void JNICALL Java_com_vread_hs_utils_UninstallObserverUtil_initUninstallObserver
        (JNIEnv *env, jobject instance, jint version, jstring url_) {

//JNIEXPORT void JNICALL
//Java_ndk_com_example_apple_hellondk_MainActivity_initUninstallObserver__ILjava_lang_String_2(
//        JNIEnv *env, jobject instance, jint version, jstring url_) {
    ...
    if (pid < 0) {
        ...
        int watchDescriptor;
        watchDescriptor = inotify_add_watch(fileDescriptor, "/data/data/com.vread.hs/lib",
                                            IN_DELETE);
        ...
        LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &b_IS_COPY),
                  (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "start observer"),
                                            &b_IS_COPY));
        LOGD("before uninstall");
        size_t readBytes = read(fileDescriptor, p_buf, sizeof(struct inotify_event));
        LOGD("after uninstall");
        ...
        FILE *p_file = fopen("/data/data/com.vread.hs", "r");
        if (p_file != NULL) {
            exit(1);
        }
        if (version < 17) {//根据版本号进行处理,4.2及以上需要加上 --user 0 提升权限
            execlp("am", "am", "start", "-a", "android.intent.action.VIEW", "-d", url,
                   (char *) NULL);
        } else {
...
//        LOG_DEBUG(tag,(*env)->NewStringUTF(env,"run in parent"));
        return;
    }
}